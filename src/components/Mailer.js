...
export default function Mailer({ isExpand, setIsExpand, setIsOpen, isOpen }) {
  ...
  const handleContact = async (e) => {
    ...
    try {
      ...
      if (response.status === 200) {
        setResp("Thank you for reaching out. We will be in touch shortly.");
        setTimeout(() => {
          setIsOpen(false);
        }, 3000);
      }
      ...
      console.error(error);
      setResp(
        "There was a problem issuing your request. If the problem perisists please reach out to info@symbioticai.ca"
      );
  ...
  return (
    <div
      className={`bg-white w-4/5 h-4/5 ${
        isMinimize
          ? "md:w-[25rem]"
    ...
    >
      <div className="rounded-t-xl text-sm font-semibold text-white flex flex-row w-full bg-[#736B90] h-10 items-center justify-between p-2">
        ...
        <form
          className={`md:${isMinimize ? "hidden" : "flex"} flex flex-col p-2 justify-between h-full`}
